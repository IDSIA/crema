...
public class ConvexHull {
	...
	public static double[][] convex2d(double[][] data) {
		Collection<Vector2D> vertices = Arrays.stream(data).map(d -> new Vector2D(new double[] { d[0], d[1] }))
				.collect(Collectors.toList());
		ConvexHull2D c2 = new MonotoneChain(false).generate(vertices);
		return Arrays.stream(c2.getVertices()).map(v -> v.toArray()).toArray(x -> new double[x][]);
	}
	...
	public static double[][] convex3d(double[][] data) {
		Point3d[] points = Arrays.stream(data).map(x -> new Point3d(x[0], x[1], x[2]))
		...
		return Arrays.stream(vertices).map(p -> new double[] { p.x, p.y, p.z }).toArray(size -> new double[size][]);
	}
