...
public class QuickHull implements ConvexHull{
	...
	public double[][] apply(double[][] vertices) {
		final int dims = vertices[0].length - (this.simplex ? 1 : 0);
	...
	private static double[][] convex2d(double[][] data) {
	...
	private static double[][] convex1d(double[][] data) {
	...
	private static double[][] convex3d(double[][] data) {
	...
	public QuickHull setSimplex(boolean simplex) {
		this.simplex = simplex;
		return this;
	}

	public static double[][] compute(double[][] vertices, boolean simplex) {
		return (new QuickHull()).setSimplex(simplex).apply(vertices);
	}
