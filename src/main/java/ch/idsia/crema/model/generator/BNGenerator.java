...
public class BNGenerator {
	...
	public static void main(String argv[]) throws Exception {
		...
		String baseFileName = "src/test/resources/polytrees/Benchmark-long"; // default file name
		...
		// This value follows the DagAlea (see Melancon;Bousque,2000)
		// suggestion,
		...
		bn.inicializeGraph(); // Inicialize a simple ordered tree as a BN
								// structure

		if ((testU.compareTo("yes") == 0)) { // for testing Uniformity
			for (int i = 0; i < bn.numberStates; i++)
				bn.repository[i] = new Matrix(); // for testing Uniformity
			bn.repository[0].inicializeTable(bn.nNodes, bn.maxDegree + 2); // for
																			// testing
																			// Uniformity
		}
		...
		if ((testU.compareTo("yes") == 0)) { // for testing Uniformity
			...
			if (maxInducedWidth != -1)
				bn.printAnalisys1(nGraphs); // IW
			if (maxInducedWidth != -1)
				bn.printAnalisys2(); // IW
			...
			bn.saveDistribution("UTest_", "doc", nGraphs, nIterations, structure, maxInducedWidth); // for
																									// testing
																									// Uniformity
...
/*	public void generateParentMatrix(BayesNet network, int parentsMatrix[][]) {
	...
	/*
	 * 
	 * //This method generate multiconnected networks with more sofisticated
	 * method //////////METHOD WITH INVERTION OPERATION
	 * ///////////////////////////////// void generateMultiConnected2(int
	 * nGraphs, int nIterations, int maxValues, String testU, String format,
	 * String baseFileName) throws Exception { int inducedWidth=1; for (int g =
	 * 0; g < nGraphs; g++) { //inicializeGraph(); // Inicialize a simple
	 * ordered tree as a BN structure System.out.println(
	 * "Generating multi-connected graph with new method:"+g); int auxTotal =
	 * maxArcs; int totalArcs=nNodes-1; // Simple tree has (nNodes-1)arcs for
	 * (int i=0;i<nIterations;i++ ) {
	 * //System.out.println("***************************************Iteration:"+
	 * i); if (verifyCoherency()== true) { boolean exist,conditionSatisfied; //
	 * auxiliary variables boolean inverted=false; boolean existInverseArc;
	 * float random=randMulti.nextFloat(); sampleArc();
	 * exist=existArc(randP,randS); existInverseArc=existArc(randS,randP);
	 * //System.out.println("\t Sorted arc("+randP+","+randS+")");
	 * //System.out.println("\t Arc exists:"+exist); //System.out.println(
	 * "\t InverseArc exists:"+existInverseArc); if (existInverseArc==false &&
	 * exist==true) { if ( random> 0.5) { /// the factor 0.5 is important to
	 * speed the convergence int aux1=randP; randP=randS; randS=aux1;
	 * addArc(randP,randS); inverted=true; removeArc(randS,randP);
	 * //System.out.println("\t Node inverted"); }else { removeArc();
	 * totalArcs--; //System.out.println("\t Node removed"); } } // end of
	 * if(false&&true)
	 * 
	 * if (existInverseArc==true && exist==false) { if ( random> 0.5) { /// the
	 * factor 0.5 is important to speed the convergence addArc(randP,randS);
	 * inverted=true; removeArc(randS,randP); //System.out.println(
	 * "\t Actual node was inverted "); }else { int aux1=randP; randP=randS;
	 * randS=aux1; removeArc(); totalArcs--; //System.out.println(
	 * "\t Actual node was removed"); } } // end of if(true&&false)
	 * 
	 * if (existInverseArc==false && exist==false) { if ( random> 0.5) { /// the
	 * factor 0.5 is important to speed the convergence int aux1=randP;
	 * randP=randS; randS=aux1; addArc(randP,randS); totalArcs++; inverted=true;
	 * //System.out.println("\t Node was inverted and added"); }else {
	 * addArc(randP,randS); totalArcs++; //System.out.println(
	 * "\t Node just added"); } } // end of if(true&&false)
	 * 
	 * //printArcs(); conditionSatisfied=false;
	 * 
	 * 
	 * //*****Condition verifyng if (inverted==true) { if (totalArcs<=auxTotal)
	 * if (followMaxDegree(maxDegree)) if(isAcyclic()) {
	 * conditionSatisfied=true; //System.out.println(
	 * "\t Structure(after invertion) is a DAG."); } } // end of if(inverted)
	 * 
	 * else if (exist==false && existInverseArc==false) { if
	 * (totalArcs<=auxTotal) if (followMaxDegree(maxDegree)) if(isAcyclic()) {
	 * conditionSatisfied=true; //System.out.println("\t Structure is a DAG.");
	 * } } // end of if(exist) else // if the node has removed, it is necessary
	 * just verify if continue connected if (isConnected()) {
	 * conditionSatisfied=true; //System.out.println(
	 * "\t Structure is connected."); }
	 * 
	 * //System.out.println("\t Condition satisfied:"+conditionSatisfied); if
	 * (conditionSatisfied==false) { if (existInverseArc==false && exist==true)
	 * { if ( random> 0.5) { /// the factor 0.5 is important to speed the
	 * convergence int aux1=randP; randP=randS; randS=aux1; addArc(randP,randS);
	 * removeArc(randS,randP); }else { addArc(); totalArcs++; } } // end of
	 * if(false&&true)
	 * 
	 * if (existInverseArc==true && exist==false) { if ( random> 0.5) { /// the
	 * factor 0.5 is important to speed the convergence int aux1=randP;
	 * randP=randS; randS=aux1; addArc(randP,randS); removeArc(randS,randP);
	 * //System.out.println("\t Node inverted and added "); }else { addArc();
	 * totalArcs++; //System.out.println("\t Node just added"); } } // end of
	 * if(true&&false)
	 * 
	 * if (existInverseArc==false && exist==false) { removeArc(randP,randS);
	 * totalArcs--; } // end of if(true&&false)
	 * 
	 * } // end of if (conditionSatisfied) //printArcs(); } // for coherency
	 * else { System.out.println(
	 * "\t********  MATRIX�S NOT COHERENT!! ********** "); System.exit(0); } //
	 * end of else //pause(); } // end of for(iteration)
	 * 
	 * if ( (testU.compareTo("yes") == 0)) { testUniformity(inducedWidth); //
	 * testUnif } else { if ((format.compareTo("xml") ==0)) {
	 * saveBNXML(baseFileName,format,g,maxValues); } if
	 * ((format.compareTo("java") == 0)) {
	 * saveBNJava(baseFileName,format,g,maxValues); } if
	 * ((format.compareTo("xmljava") == 0)) {
	 * saveBNXMLJava(baseFileName,format,g,maxValues); } } } // end of
	 * for(graphs) } // end of generateMultiConnected2()
	 * 
	 * //////////METHOD WITH INVERTION OPERATION WITH INDUCED WIDTH CONSTRAINT
	 * ///////////////////////////////// void generateMwithIW2(int nGraphs, int
	 * nIterations, int maxValues, String testU, String format, String
	 * baseFileName, int maxIW) throws Exception { int inducedWidth=1;
	 * System.out.println("Maximum Induced-width is:"+maxIW); int max=0; for
	 * (int g = 0; g < nGraphs; g++) { //inicializeGraph(); // Inicialize a
	 * simple ordered tree as a BN structure //System.out.println(
	 * "Generating multi-connected graph with new method (using IW):"+g); int
	 * auxTotal = maxArcs; int totalArcs=nNodes-1; // Simple tree has
	 * (nNodes-1)arcs for (int i=0;i<nIterations;i++ ) {
	 * //System.out.println("***************************************Iteration:"+
	 * i); if (verifyCoherency()== true) { boolean exist,conditionSatisfied; //
	 * auxiliary variables boolean inverted=false; boolean existInverseArc;
	 * float random=randMulti.nextFloat(); sampleArc();
	 ...
	 * //*****Condition verifyng if (inverted==true) { if (totalArcs<=auxTotal)
	 * if (followMaxDegree(maxDegree)) if(isAcyclic()) {
	 * loadToEmBayes("loadingBN"); // loadind network to EmBayes for computing
	 * induced-width int aux=inducedWidth; inducedWidth=getInducedWidth(); if
	 * (inducedWidth<=maxIW) conditionSatisfied=true; else inducedWidth=aux;
	 * //conditionSatisfied=true; //System.out.println(
	 * "\t Structure(after invertion) is a DAG."); } } // end of if(inverted)
	 * 
	 * else if (exist==false && existInverseArc==false) { if
	 ...
	 * (inducedWidth<=maxIW) conditionSatisfied=true; else inducedWidth=aux;
	 * //conditionSatisfied=true; //System.out.println("\t Structure is a DAG."
	 * ); } } // end of if(exist) else // if the node has removed, it is
	 * necessary just verify if continue connected if (isConnected()) {
	 * loadToEmBayes("loadingBN"); // loadind network to EmBayes for computing
	 * induced-width inducedWidth=getInducedWidth(); if (inducedWidth<=maxIW)
	 * conditionSatisfied=true; //else System.out.println(
	 * "\t **** Removeu um arco e ainda seu IW aumentou!!");
	 * //conditionSatisfied=true; //System.out.println(
	 * "\t Structure is connected."); }
	 * 
	 * //System.out.println("\t Condition satisfied:"+conditionSatisfied); if
	 ...
	 * end of else //pause(); } // end of for(iteration)
	 * 
	 * if ( (testU.compareTo("yes") == 0)) { testInducedWidth(g,inducedWidth);
	 * //if (inducedWidth>max) max=inducedWidth; testUniformity(inducedWidth);
	 * // testUnif } else { if ((format.compareTo("xml") ==0)) {
	 * saveBNXML(baseFileName,format,g,maxValues); } if
	 * ((format.compareTo("java") == 0)) {
	 * saveBNJava(baseFileName,format,g,maxValues); } if
	 * ((format.compareTo("xmljava") == 0)) {
	 * saveBNXMLJava(baseFileName,format,g,maxValues); } } } // end of
	 * for(graphs) System.out.println("\t Maximum induced-width: "+max); // IW }
	 * // end of generateTest2()
	 * 
	 * //This method is similar to void generateMulticonect, except that add the
	 * induced-width constraint void generateMwithIW(int nGraphs, int
	 * nIterations, int maxValues, String testU, String format, String
	 * baseFileName, int maxIW)throws Exception{ int inducedWidth=1;
	 * System.out.println("Maximum Induced-width is:"+maxIW); for (int g = 0; g
	 * < nGraphs; g++) { //inicializeGraph(); // Inicialize a simple ordered
	 * tree as a BN structure (optional) System.out.println(
	 * "Generating multi-connected test graph:"+g); int auxTotal = maxArcs; int
	 * totalArcs=nNodes-1; // Simple tree has (nNodes-1)arcs //inducedWidth=1;
	 * // induced-width must be initialized, if we initializeGraph() everytime.
	 * for (int i=0;i<nIterations;i++ ) { //if (verifyCoherency()== true) {
	 * boolean exist,conditionSatisfied; // auxiliary variables //boolean
	 * inverted=false; sampleArc(); exist=existArc(randP,randS); if
	 * (exist==false) { addArc(randP,randS); totalArcs++; //System.out.println(
	 * "\t Node just added"); }else { // arc exists removeArc(); totalArcs--;
	 * ///System.out.println("\t Arc removed for verifying"); } // end of else
	 * //printArcs();
	 * 
	 * conditionSatisfied=false; if (exist==false) { if (totalArcs<=auxTotal) if
	 * (followMaxDegree(maxDegree)) if(isAcyclic()) {
	 * loadToEmBayes("loadingBN"); // loadind network to EmBayes for computing
	 * induced-width int aux=inducedWidth; inducedWidth=getInducedWidth(); if
	 * (inducedWidth<=maxIW) conditionSatisfied=true; else inducedWidth=aux;
	 * //if (inducedWidth<(auxIW)) System.out.println(
	 * "\t *************** Adicionou arco e IW diminuiu!."); //if
	 * (inducedWidth==(auxIW)) System.out.println(
	 * "\t Adicionou arco e IW aumentou nada."); //if (inducedWidth==(auxIW+1))
	 * System.out.println("\t Adicionou arco e IW aumentou 1."); //if
	 * (inducedWidth==(auxIW+2)) System.out.println(
	 * "\t **** Adicionou arco e IW aumentou 2!"); } } // end of if(exist) else
	 * // if the node has removed, it is necessary just verify if continue
	 * connected if (isConnected()==true) { int auxIW=inducedWidth;
	 * loadToEmBayes("loadingBN"); // loadind network to EmBayes for computing
	 * induced-width inducedWidth=getInducedWidth(); //System.out.println(
	 * "\t Removeu um arco."); //if (inducedWidth<=maxIW)
	 * conditionSatisfied=true; if (inducedWidth>auxIW) System.out.println(
	 * " ******** Removeu um arco e ainda seu IW aumentou!!"); //if
	 * (inducedWidth==auxIW) System.out.println(
	 * " Removeu um arco e o induced width permaneceu igual."); //if
	 * (inducedWidth==(auxIW-1)) System.out.println(
	 * " Removeu-se arco e IW diminuiu 1."); conditionSatisfied=true;
	 * //System.out.println("\t Structure is connected."); } if
	 * (conditionSatisfied==false) { if (exist) { addArc(); totalArcs++;
	 * //System.out.println("\t Arc re-added"); }else { removeArc();
	 * totalArcs--; //System.out.println("\t Arc ad-removed"); } } // end of if
	 * (conditionSatisfied)
	 * 
	 * 
	 * //System.out.println("\n Pausando, para verificar!"); //printArcs(); //}
	 * // for coherency //else { // System.out.println(
	 * "\t********  MATRIX�S NOT COHERENT!! ********** "); // System.exit(0);
	 * //} // end of else //pause();
	 * 
	 * 
	 ...
	 * 
	 * if ( (testU.compareTo("yes") == 0)) { testInducedWidth(g,inducedWidth);
	 * testUniformity(inducedWidth); // testUnif } else { if
	 * ((format.compareTo("xml") ==0)) {
	 * saveBNXML(baseFileName,format,g,maxValues); } if
	 * ((format.compareTo("java") == 0)) {
	 * saveBNJava(baseFileName,format,g,maxValues); } if
	 * ((format.compareTo("xmljava") == 0)) {
	 * saveBNXMLJava(baseFileName,format,g,maxValues); } if
	 * ((format.compareTo("loadEmB") == 0)) { loadToEmBayes("bnLink");
	 * //ejb.loadNetwork(st); ejb.mainEmBayes(ejb); } }
	 * 
	 * } // end of for(graphs)
	 * 
	 * } // end of generateTestStructure()
	 * 
	 * /////////// Generating method with mixing strategy (see article) void
	 * generateMixedE(int nGraphs, int nIterations, int maxValues, String testU,
	 * String format, String baseFileName, int maxIW)throws Exception{ int
	 * inducedWidth=1; int totalArcs=nNodes-1; // Simple tree has (nNodes-1)arcs
	 * int auxTotal = maxArcs; double rate=0.5; // transition rate double
	 * random; System.out.println("Maximum Induced-width is:"+maxIW); for (int g
	 * = 0; g < nGraphs; g++) { System.out.println(
	 * "Generating multi-connected graph (with mixed strategy):"+g); for (int
	 * i=0;i<nIterations;i++ ) { //System.out.println(
	 * "***************************************Iteration(mixed):"+i);
	 * random=rand.nextDouble(); if (totalArcs==nNodes-1) { // this condition
	 * verify if graph is a polytree if ( random>=rate) { // AR operation
	 * boolean exist; sampleArc(); if (followMaxDegree(maxDegree-1))
	 * exist=existArc(randP,randS); else exist=true; if (exist==false) { int
	 * remove[]=findArcToRemove(randP,randS); if (randPolytree.nextFloat() >
	 * 0.5) { // the factor 0.5 is important for uniformity addArc(randS,randP);
	 * // note that arc was inverted!! randP and randS are atualized
	 * removeArc(remove[0],remove[1]); int auxIW=inducedWidth;
	 * loadToEmBayes("loadingBN"); // IW inducedWidth=getInducedWidth(); // IW
	 * if (inducedWidth>maxIW) { // new graph rejected removeArc();
	 * addArc(remove[0],remove[1]); // add the arc that was removed
	 * inducedWidth=auxIW; } } else { addArc(randP,randS);
	 * removeArc(remove[0],remove[1]); int auxIW=inducedWidth;
	 * loadToEmBayes("loadingBN"); // IW inducedWidth=getInducedWidth(); // IW
	 * if (inducedWidth>maxIW) { // new graph rejected removeArc();
	 * addArc(remove[0],remove[1]); inducedWidth=auxIW; } } } // end of
	 * if(exist) } // end of if(random>=rate) else { // A or R operation boolean
	 * exist,conditionSatisfied; // auxiliary variables sampleArc();
	 * exist=existArc(randP,randS); conditionSatisfied=false; if (exist==false)
	 * { addArc(randP,randS); totalArcs++; if (totalArcs<=auxTotal) if
	 * (followMaxDegree(maxDegree)) if(isAcyclic()) { int auxIW=inducedWidth;
	 * loadToEmBayes("loadingBN"); // loadind network to EmBayes for computing
	 * induced-width inducedWidth=getInducedWidth(); if (inducedWidth<=maxIW)
	 * conditionSatisfied=true; else inducedWidth=auxIW; } // end of if } // end
	 * of if(exist) if (exist==false) { if (conditionSatisfied==false) {
	 * removeArc(); totalArcs--; } } // end of if (conditionSatisfied) } // end
	 * of else (A or R operation) } // end of if(polytree) else { // actual
	 * graph is multiconnected, then use A or R operation boolean
	 * exist,conditionSatisfied; // auxiliary variables int aux=inducedWidth; //
	 * aux keep the initial induced-width of the multiconnected graph
	 * sampleArc(); exist=existArc(randP,randS); if (exist==false) {
	 * addArc(randP,randS); totalArcs++; }else { // arc exists removeArc();
	 * totalArcs--; } // end of else conditionSatisfied=false; if (exist==false)
	 * { // if the arc does not exist, naturaly the arc is added and the remain
	 * graph is multiconnected if (totalArcs<=auxTotal) if
	 * (followMaxDegree(maxDegree)) if(isAcyclic()) {
	 * loadToEmBayes("loadingBN"); // loadind network to EmBayes for computing
	 * induced-width inducedWidth=getInducedWidth(); if (inducedWidth<=maxIW)
	 * conditionSatisfied=true; else inducedWidth=aux; } // end of if } // end
	 * of if(exist) else {// if the node has removed, it is necessary just
	 * verify if continue connected and if it is multiconnected if
	 * (isConnected()==true) { loadToEmBayes("loadingBN"); // loadind network to
	 * EmBayes for computing induced-width inducedWidth=getInducedWidth();
	 * conditionSatisfied=false; if (inducedWidth<=maxIW) { if
	 * (totalArcs==nNodes-1) { // verify if graph is a polytree if (random<rate)
	 * conditionSatisfied=true; // accept the polytree else inducedWidth=aux; }
	 * // end of if(polytree) else conditionSatisfied=true; // resultant graph
	 * is multiconnected => accept //if (inducedWidth>aux) // if this condition
	 * is satisfied, its a patologic case!! //System.out.println(
	 * " ******** Removeu um arco e ainda seu IW aumentou!!"); } else
	 * inducedWidth=aux; // reject the graph } } if (conditionSatisfied==false)
	 * { if (exist) { addArc(); totalArcs++; }else { removeArc(); totalArcs--; }
	 * } // end of if (conditionSatisfied) } //end of else } // end of
	 * for(iteration)
	 * 
	 * if ( (testU.compareTo("yes") == 0)) { testInducedWidth(g,inducedWidth);
	 * testUniformity(inducedWidth); // testUnif } else { if
	 ...
	 * ejb.mainEmBayes(ejb); } }
	 * 
	 * } // end of for(graphs) } // end of generateTest3()
	 * 
	 * void generateMixedER(int nGraphs, int nIterations, int maxValues, String
	 * testU, String format, String baseFileName, int maxIW)throws Exception{
	 * int inducedWidth=1; int totalArcs=nNodes-1; // Simple tree has
	 * (nNodes-1)arcs int auxTotal = maxArcs; double random; double rate=0.1;
	 * double rate2=0.05; System.out.println("Maximum Induced-width is:"+maxIW);
	 * int statistics[]= new int[6]; for (int g = 0; g < nGraphs; g++) {
	 * System.out.println("Generating multi-connected Test3 graph:"+g); //
	 * ********************************************** Iteraction to obtain one
	 * graph ********************* for (int i=0;i<nIterations;i++ ) {
	 * random=rand.nextDouble();
	 * //********************************************** is
	 * polytree****************************** if (totalArcs==nNodes-1) { // this
	 * condition verify if graph is a polytree if ( random<=rate) { // AR
	 * operation boolean exist; sampleArc(); if (followMaxDegree(maxDegree-1))
	 * exist=existArc(randP,randS); else exist=true; if (exist==false) { int
	 * remove[]=findArcToRemove(randP,randS); if (randPolytree.nextFloat() >
	 * 0.5) { // the factor 0.5 is important for uniformity addArc(randS,randP);
	 * // note that arc was inverted!! randP and randS are atualized
	 ...
	 * addArc(remove[0],remove[1]); inducedWidth=auxIW; } } } // end of
	 * if(exist) } // end of if(random<rate) else { // A or R operation boolean
	 * exist,conditionSatisfied; // auxiliary variables int
	 * initialPolytreeP[][]=receiveMatrix(parentMatrix,nNodes,maxDegree+2); int
	 * initialPolytreeS[][]=receiveMatrix(sonMatrix,nNodes,maxDegree+2); int
	 * auxIW=inducedWidth; random=0.0; int count=0; while(random<=rate2) {
	 * count++; sampleArc(); exist=existArc(randP,randS); if (exist==false) {
	 * addArc(randP,randS); totalArcs++; }else { // arc exists removeArc();
	 * totalArcs--; } // end of else
	 * 
	 * conditionSatisfied=false; if (exist==false) { if (totalArcs<=auxTotal) if
	 * (followMaxDegree(maxDegree)) if(isAcyclic()) { conditionSatisfied=true; }
	 * } // end of if(exist) else if (isConnected()==true) { // if the node has
	 * removed, it is necessary just verify if continue connected
	 * conditionSatisfied=true; } if (conditionSatisfied==false) { if (exist) {
	 * addArc(); totalArcs++; }else { removeArc(); totalArcs--; } } // end of if
	 * (conditionSatisfied==false) random=rand.nextDouble(); } // end of while
	 * if ( totalArcs!=(nNodes-1) ) { // graph must be multiconnected
	 * loadToEmBayes("loadingBN"); // loadind network to EmBayes for computing
	 * induced-width inducedWidth=getInducedWidth(); if (inducedWidth<=maxIW){
	 * if (count>2) statistics[1]++; conditionSatisfied=true; } else
	 * conditionSatisfied=false; } else conditionSatisfied=false; if
	 * (conditionSatisfied==false) { statistics[0]++;
	 * parentMatrix=receiveMatrix(initialPolytreeP,nNodes,maxDegree+2);
	 * sonMatrix=receiveMatrix(initialPolytreeS,nNodes,maxDegree+2);
	 * inducedWidth=auxIW; totalArcs=nNodes-1; } } // end of else (A or R
	 * operation)
	 * 
	 * 
	 * } // end of if(polytree)
	 * 
	 * //********************************************** is multiconnected
	 * ************************ else { // actual graph is multiconnected, then
	 * use A or R operation boolean exist,conditionSatisfied; // auxiliary
	 * variables int
	 * initialMulticonnectedP[][]=receiveMatrix(parentMatrix,nNodes,maxDegree+2)
	 * ; int
	 * initialMulticonnectedS[][]=receiveMatrix(sonMatrix,nNodes,maxDegree+2);
	 * int auxIW=inducedWidth; int auxTotalArcs=totalArcs; int count=0;
	 * random=0.0; while(random<=rate2) { count++; sampleArc();
	 * exist=existArc(randP,randS); if (exist==false) { addArc(randP,randS);
	 * totalArcs++; }else { // arc exists removeArc(); totalArcs--; } // end of
	 * else conditionSatisfied=false; if (exist==false) { // if the arc does not
	 * exist, naturaly the arc is added and the remain graph is multiconnected
	 * if (totalArcs<=auxTotal) if (followMaxDegree(maxDegree)) if(isAcyclic())
	 * { conditionSatisfied=true; } // end of if } // end of if(exist) else {//
	 * if the node has removed, it is necessary just verify if continue
	 * connected and if it is multiconnected if (isConnected()==true) {
	 * conditionSatisfied=true; } } if (conditionSatisfied==false) { if (exist)
	 * { addArc(); totalArcs++; }else { removeArc(); totalArcs--; } } // end of
	 * if (conditionSatisfied) random=rand.nextDouble(); } //end of while (A or
	 * R) operation if ( (totalArcs!=(nNodes-1)) ) { // graph is multiconnected,
	 * verifying IW loadToEmBayes("loadingBN"); // loadind network to EmBayes
	 * for computing induced-width inducedWidth=getInducedWidth(); if
	 * (inducedWidth<=maxIW){ if (count>2) statistics[3]++;
	 * conditionSatisfied=true; } else conditionSatisfied=false; } else { //
	 * graph is polytree, random=rand.nextDouble(); if (random>rate) { //
	 * polytree accepted, verifying IW loadToEmBayes("loadingBN"); // loadind
	 * network to EmBayes for computing induced-width
	 * inducedWidth=getInducedWidth(); if (inducedWidth<=maxIW) { if (count>2)
	 * statistics[3]++; conditionSatisfied=true; } else
	 * conditionSatisfied=false; } else conditionSatisfied=false; // polytree
	 * rejected } if (conditionSatisfied==false) { statistics[2]++;
	 * parentMatrix=receiveMatrix(initialMulticonnectedP,nNodes,maxDegree+2);
	 * sonMatrix=receiveMatrix(initialMulticonnectedS,nNodes,maxDegree+2);
	 * inducedWidth=auxIW; totalArcs=auxTotalArcs; } } // end of else (graph is
	 * multiconnected) } // end of for(iteration)
	 * 
	 * if ( (testU.compareTo("yes") == 0)) { //loadToEmBayes("loadingBN"); //
	 * loadind network to EmBayes for computing induced-width
	 * //inducedWidth=getInducedWidth(); //if (inducedWidth>maxIW) throw(new
	 * Exception("Error at IW verification!!!"));
	 * testInducedWidth(g,inducedWidth); testUniformity(inducedWidth); //
	 * testUnif } else { if ((format.compareTo("xml") ==0)) {
	 * saveBNXML(baseFileName,format,g,maxValues); } if
	 * ((format.compareTo("java") == 0)) {
	 * saveBNJava(baseFileName,format,g,maxValues); } if
	 ...
	 * } // end of for(graphs)
	 * 
	 * System.out.println("1) Rejected Loop Operation (polytree):"
	 * +statistics[0]); System.out.println(
	 * "1) Accepted Loop Operation after two jump(polytree):"+statistics[1]);
	 * System.out.println("2) Rejected Loop Operation (multiconnected):"
	 * +statistics[2]); System.out.println(
	 * "2) Accepted Loop Operation after two jump(multiconnected):"
	 * +statistics[3]); } // end of generateTest3()
	 * 
	 */
